...
SELECT models.name, brands.name, brands.headquarters FROM models JOIN brands ON brands.name=models.brand_name WHERE models.name = 'Mustang';
...
SELECT m.name, m.brand_name, b.founded FROM models AS m LEFT JOIN brands AS b ON (b.name = m.brand_name) WHERE m.year = 1960;
...
SELECT b.name, m.name, m.year, b.discontinued, b.discontinued - m.year AS years_until_brand_discontinued FROM models AS M LEFT JOIN brands AS b ON b.name = m.brand_name WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year;
